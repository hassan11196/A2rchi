...
class FlaskAppWrapper(object):
    ...
    def sso_callback(self):
        ...
        try:
            ...
            # (At CERN SSO sub == preferred_username, so Mattermost-context
            # lookups by username still resolve.)
    ...
    def mcp_callback(self):
        ...
        # requests, so chat-time lookups find the token. (At CERN SSO
        # sub == preferred_username, so Mattermost lookups still resolve.)
        user_id = session.get('user', {}).get('id', '')
